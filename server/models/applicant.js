...
async function provisionTables() {
  ...
  try {
    await pool.query(`ALTER TABLE recruitment_applicants ADD COLUMN physical_screening_status VARCHAR(150) AFTER medical_status`);
  } catch (error) {
    if (error && error.code !== 'ER_DUP_FIELDNAME') throw error;
  }

  try {
    await pool.query(`ALTER TABLE recruitment_applicants ADD COLUMN previous_status VARCHAR(150) AFTER status`);
  } catch (error) {
    if (error && error.code !== 'ER_DUP_FIELDNAME') throw error;
  }
...
async function insertRecruitmentApplicant(data) {
  ...
  const values = [
    ...
    data.physical_screening_status || null,
  ...
  const [result] = await pool.execute(
    `INSERT INTO recruitment_applicants (
      ...
      requirements_status, final_interview_status, medical_status, physical_screening_status, status_remarks, applicant_remarks,
    ...
    await pool.execute(
      `UPDATE recruitment_applicants
       SET referred_by=IFNULL(?, referred_by), last_name=IFNULL(?, last_name), first_name=IFNULL(?, first_name), ext=IFNULL(?, ext), middle_name=IFNULL(?, middle_name), gender=IFNULL(?, gender), size=IFNULL(?, size), date_of_birth=IFNULL(?, date_of_birth),
           ...
           previous_status=IF(? IS NOT NULL AND ? <> status, status, previous_status),
           status=IFNULL(?, status),
           requirements_status=IFNULL(?, requirements_status), final_interview_status=IFNULL(?, final_interview_status), medical_status=IFNULL(?, medical_status), physical_screening_status=IFNULL(?, physical_screening_status), status_remarks=IFNULL(?, status_remarks), applicant_remarks=IFNULL(?, applicant_remarks),
      ...
      [
        ...
        normalize(data.physical_screening_status),
