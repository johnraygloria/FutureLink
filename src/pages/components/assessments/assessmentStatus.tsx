...
const Assessment: React.FC<AssessmentProps> = ({ applicantNo, showApplicantHeader = true }) => {
  ...
  return (
    <div className="w-full">
      <div className={`w-full ${isCompact ? '' : 'p-6'}`}>
        <div className={`${isCompact ? 'bg-transparent shadow-none overflow-visible' : 'bg-black/20 rounded-2xl shadow-lg border border-white/10 overflow-hidden'}`}>
          {!isCompact && (
            <div className="flex items-center justify-between p-6 border-b border-white/10">
              <div className="flex items-center space-x-4">
                <h1 className="text-2xl font-bold text-white">Employee Screening & Assessment</h1>
                <div className="h-6 w-px bg-white/10"></div>
                <span className="text-sm text-text-secondary">Assessment Management</span>
              </div>
            </div>
          )}
          ...
          <div className={`${isCompact ? 'p-0' : 'p-6'}`}>
            {showApplicantHeader && applicantData.NO && applicantData.FIRST_NAME && (
              <div className="mb-6 bg-gradient-to-r from-custom-teal/10 to-blue-500/5 rounded-xl p-6 border border-custom-teal/20 backdrop-blur-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-custom-teal">Current Applicant</h3>
                  <div className="flex items-center space-x-2">
                    <span className="px-3 py-1 bg-custom-teal/10 text-custom-teal rounded-full text-sm font-medium border border-custom-teal/20">
                      Active Assessment
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="flex flex-col">
                    <span className="text-xs uppercase font-medium text-text-secondary">Full Name</span>
                    <span className="text-base font-semibold text-white">
                      {applicantData.FIRST_NAME} {applicantData.LAST_NAME}
                    </span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-xs uppercase font-medium text-text-secondary">Position</span>
                    <span className="text-base font-semibold text-white">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs uppercase font-medium text-text-secondary">Location</span>
                    <span className="text-base font-semibold text-white">
            ...
            <form onSubmit={handleSubmit} className={`${isCompact ? 'space-y-4' : 'space-y-6'}`}>
              <div className={`grid grid-cols-1 lg:grid-cols-2 ${isCompact ? 'gap-4' : 'gap-6'}`}>
                <div className="flex flex-col gap-3 bg-white/5 rounded-xl p-4 border border-white/10 shadow-sm backdrop-blur-sm">
                    ...
                    <div className="mt-2 pt-2 border-t border-white/10">
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {selectedCompanies.map((clientName, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-custom-teal text-white shadow-sm"
                          >
                            {clientName}
                          </span>
                        ))}
                ...
                <div className="flex flex-col gap-2 bg-white/5 rounded-xl p-4 border border-white/10 shadow-sm backdrop-blur-sm">
                  <label className="text-sm font-bold text-white flex items-center gap-2 uppercase tracking-wide">
                  ...
                  <input
                    ...
                    className="w-full border border-white/10 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-custom-teal focus:border-custom-teal bg-white/5 text-white placeholder-text-secondary/30 transition-colors font-medium"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2 bg-white/5 rounded-xl p-4 border border-white/10 shadow-sm backdrop-blur-sm">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full border border-white/10 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-custom-teal focus:border-custom-teal bg-white/5 text-white placeholder-text-secondary/30 resize-none transition-colors"
                />
              </div>

              <div className={`flex items-center justify-between ${isCompact ? 'pt-4 border-t border-white/10' : 'pt-6 border-t border-white/10 mx-6'} py-4`}>
                <div className="flex items-center space-x-4">
                  <button
                    type="submit"
                    disabled={loading}
                    className={`${isCompact ? 'px-5 py-2.5 text-sm' : 'px-8 py-3'} bg-custom-teal text-white font-semibold rounded-lg shadow-lg hover:shadow-xl focus:outline-none border border-custom-teal/50 hover:bg-custom-teal/80 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2`}
                  >
                    {loading ? (
                      <>
                ...
                {status && (
                  <div className={`flex items-center space-x-2 ${isCompact ? 'px-4 py-2 text-sm rounded-lg' : 'px-5 py-2.5 rounded-lg'} shadow-sm backdrop-blur-sm ${status === 'Ok'
                    ...
                    <span className="font-semibold">{status === 'Ok' ? 'Saved Successfully' : status}</span>
                  </div>
                )}
              </div>
            </form>
