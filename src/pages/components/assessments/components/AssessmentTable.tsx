...
import { formatAppliedDate, getUserInitials, ADDITIONAL_ASSESSMENT_COLUMNS, mapUserToDisplayFormat } from "../utils/assessmentUtils";
...
const AssessmentTable: React.FC<AssessmentTableProps> = ({ users, selectedUser, onUserClick, isLoading, hasActiveFilters = false }) => (
    ...
    <table className="min-w-full border-separate border-spacing-0">
      ...
      <tbody className="divide-y divide-white/5">
        {isLoading && (
          Array.from({ length: 6 }).map((_, idx) => (
            <tr key={`skeleton-${idx}`} className="animate-pulse">
              ...
              {ADDITIONAL_ASSESSMENT_COLUMNS.map((_, colIdx) => (
        ...
        {!isLoading && users.length === 0 && (
          <tr>
            <td colSpan={4 + ADDITIONAL_ASSESSMENT_COLUMNS.length} className="px-6 py-20 text-center text-text-secondary">
