...
- **TEMPORARY empty-row cleanup — remove once the DB is clean.** Bulk import now (a) skips fully-empty placeholder rows (a NO with no name/data — both in the Excel parser and `bulkUpsertApplicants`), and (b) after each import calls `POST /api/applicants/cleanup-empty` → `deleteEmptyApplicants()`, which deletes rows where every person/data column is blank and every doc flag is 0 (verified to match exactly the 481 legacy empties, names intact). This is a stop-gap to sweep out accumulated empties; delete the endpoint, route, model fn, and the frontend call in `ImportExcelModal` once the data is confirmed clean.
